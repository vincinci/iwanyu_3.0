  ...
  return (
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 group overflow-hidden border border-gray-100">
      ...
      <div className="relative aspect-square overflow-hidden">
        ...
        >
          <HeartIcon className="h-3 w-3 text-brand-charcoal-600 hover:text-brand-charcoal-700" />
        </button>
      </div>
      ...
      <div className="p-2.5">
        {/* Vendor */}
        <p className="text-xs text-brand-charcoal-500 mb-1 font-medium">{vendorName}</p>
        
        {/* Product Name */}
        <Link href={`/products/${product.slug}`}>
          <h3 className="text-sm font-medium text-brand-charcoal-700 mb-1.5 hover:text-brand-charcoal-800 transition-colors line-clamp-2 leading-tight">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="flex items-center gap-1 mb-2">
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
              <span key={i}>
                {i < 4 ? (
                  <StarSolidIcon className="h-2.5 w-2.5 text-brand-yellow-500" />
                ) : (
          ...
          <span className="text-xs text-brand-charcoal-500">4.0</span>
        </div>

        {/* Price */}
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-sm font-semibold text-brand-charcoal-700">
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-brand-yellow-500 text-brand-charcoal-700 rounded-md font-semibold hover:bg-brand-yellow-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
        >
          {isAddingToCart ? (
            <div className="animate-spin rounded-full h-3 w-3 border-2 border-brand-charcoal-700 border-t-transparent"></div>
          ) : (
    ...
    <section className="py-4 bg-white">
      <div className="max-w-7xl mx-auto px-3">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-semibold text-brand-charcoal-700 mb-1">
              Featured Products
            </h2>
          ...
          <Link
            href="/products"
            className="text-xs text-brand-blue-500 hover:text-brand-blue-600 font-medium"
          >
            View All →
          </Link>
        </div>
        ...
        {products.length > 0 ? (
          ...
          <div className="text-center py-12">
            <p className="text-brand-charcoal-600 text-lg">No products available at the moment.</p>
            <p className="text-brand-charcoal-500 text-sm mt-2">Check back soon for new products!</p>
          </div>
        )}
